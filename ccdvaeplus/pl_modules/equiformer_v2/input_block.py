...
class EdgeDegreeEmbedding(torch.nn.Module):
    ...
    def __init__(
        ...
        self.decoder = use_latent_embedding
        if self.use_atom_edge_embedding:
            ...
            if self.decoder:
                self.edge_channels_list[0] = self.edge_channels_list[0] + 2 * self.edge_channels_list[1] + 2 * self.edge_channels_list[-1]
            else:
    ...
    def forward(
        ...
        if self.use_atom_edge_embedding:
            ...
            if self.decoder:
                x_edge = torch.cat((edge_distance, z[edge_index[0]], z[edge_index[1]], source_embedding, target_embedding), dim=1)
            else:
