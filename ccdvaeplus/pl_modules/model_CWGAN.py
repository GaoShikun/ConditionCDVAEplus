...
from ccdvaeplus.pl_modules.embeddings import KHOT_EMBEDDINGS
from scripts.eval_utils import composition2atom_types
...
class BaseModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        base_params = list(self.encoder.parameters()) + \
        ...
        optimizer_base = hydra.utils.instantiate(
            self.hparams.optim.optimizer, params=base_params, _convert_="partial"
        )

        # Set the optimizer separately for Gen and Dis
        optimizer_Gen = hydra.utils.instantiate(
            self.hparams.optim.optimizer_gen,
            params=self.Gen.parameters(),
            _convert_="partial"
        )
        optimizer_Dis = hydra.utils.instantiate(
            self.hparams.optim.optimizer_dis,
            params=self.Dis.parameters(),
            _convert_="partial"
        )

        optimizers = [optimizer_base, optimizer_Gen, optimizer_Dis]

        if not self.hparams.optim.use_lr_scheduler:
            return optimizers

        # if self.current_epoch < 0:
        #     # In the first 1000 epochs of training, only the optimizer and the corresponding scheduler of the basic part are returned.
        #     scheduler = hydra.utils.instantiate(
        #         self.hparams.optim.lr_scheduler, optimizer=optimizer_base
        #     )
        #     return {
        ...
        #     }
        # else:
        #     # After 10 epochs, all optimizers and their corresponding schedulers are returned (here we assume that different schedulers are configured for different optimizers, similar to the previous example)

        scheduler_base = hydra.utils.instantiate(
            self.hparams.optim.lr_scheduler, optimizer=optimizer_base
        )
        scheduler_gen = hydra.utils.instantiate(
            self.hparams.optim.lr_scheduler_gen, optimizer=optimizer_Gen
        )
        scheduler_dis = hydra.utils.instantiate(
            self.hparams.optim.lr_scheduler_dis, optimizer=optimizer_Dis
        )
        return ({"optimizer": optimizer_base,"lr_scheduler": {"scheduler": scheduler_base,"monitor": "val_loss","strict": False}},
                {"optimizer": optimizer_Gen,"lr_scheduler": {"scheduler":scheduler_gen,"monitor": "g_loss","strict": False}},
                {"optimizer": optimizer_Dis,"lr_scheduler": {"scheduler": scheduler_dis,"monitor": "d_loss","strict": False}})


class CCDVAEPLUS(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.con_model = hydra.utils.instantiate(self.hparams.lmf)

        self.decoder = hydra.utils.instantiate(self.hparams.decoder)

        self.Gen = hydra.utils.instantiate(self.hparams.generator)
        self.Dis = hydra.utils.instantiate(self.hparams.discriminator)
    ...
    def load_structure(self, csv_path):
        structure = pd.read_csv(csv_path)['cif'].tolist()
        structures = [Structure.from_str(cif_file, fmt='cif') for cif_file in structure]
        return structures
    ...
    def get_atom_type_embedding(self, atom_index):
        def get_atom_vec(index):
            if index == 0:
                return torch.zeros(50)
            else:
                atom_vector = self.atom_similarity.get_atom_vector(index)  # atomic index
                return torch.tensor(atom_vector)

        atom_vectors = list(map(lambda index: get_atom_vec(index), atom_index))
        return atom_vectors
    ...
    @torch.no_grad()
    def langevin_dynamics(self, z, ld_kwargs, gt_num_atoms=None, gt_atom_types=None, prop=None, formula=None, down_sample=10):
        """
        ...
        if self.condition_prop:
            prop = prop.repeat_interleave(z.size(0), dim=0)
            prop = self.scaler.transform(prop)
            z_prop = self.Gen(z, prop)
            pre_prop = self.fc_property(z_prop)

            mse = ((prop - pre_prop) ** 2).mean(dim=1)
            _, idx = torch.topk(mse, k=z_prop.size(0)/down_sample, largest=False)
            z_prop = z_prop[idx]
        ...
        if self.condition_formula:
            cur_atom_types = torch.tensor(composition2atom_types(formula), device=z.device)
            num_atoms = torch.tensor(cur_atom_types.size(), device=z.device)
            cur_atom_types = cur_atom_types.repeat_interleave(z.size(0), dim=0)
            num_atoms = num_atoms.repeat_interleave(z.size(0), dim=0)
            z_formula = self.Gen(z, atomtype=cur_atom_types, natoms=num_atoms)
            z_prop = self.con_model(z_formula, atom_type=cur_atom_types, num_atoms=num_atoms)
            _, _, lengths, angles, _ = self.decode_stats(
                z_prop, num_atoms)
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        z_condition = self.con_model(z, batch.y)

        (pred_num_atoms, pred_lengths_and_angles, pred_lengths, pred_angles,
         pred_composition_per_atom) = self.decode_stats(
            z_condition, batch.num_atoms, batch.lengths, batch.angles, teacher_forcing)
        ...
        pred_cart_coord_diff, pred_atom_types = self.decoder(
            z_condition, noisy_frac_coords, rand_atom_types, batch.num_atoms, pred_lengths, pred_angles)
    ...
    def gan_property_loss(self, z_fake, classes):
        return F.mse_loss(self.fc_property(z_fake), classes)
    ...
    def training_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        teacher_forcing = (
                current_epoch <= self.hparams.teacher_forcing_max_epoch)
        outputs = self(batch, teacher_forcing, training=True)
        z = outputs['z'].detach()

        base_optimizer = self.optimizers()[0]
        if current_epoch <= self.hparams.firststep_epoch:
            self.toggle_optimizer(base_optimizer)
            log_dict, loss = self.compute_stats(batch, outputs, prefix='train')

            self.manual_backward(loss)
            self.clip_gradients(base_optimizer, gradient_clip_val=0.5, gradient_clip_algorithm="value")
            base_optimizer.step()
            base_optimizer.zero_grad()
            self.untoggle_optimizer(base_optimizer)
            ...
            gen_optimizer = self.optimizers()[1]
            dis_optimizer = self.optimizers()[2]

            noise = torch.randn((z.size(0), z.size(1)), device=self.device).type_as(z)
            gen_output = self.Gen(noise, batch.y)

            self.toggle_optimizer(dis_optimizer)

            # Gradient penalty
            alpha = torch.rand(z.size(0), 1, device=self.device)
            interpolates = (alpha * z + (1 - alpha) * gen_output.detach()).requires_grad_(True)
            d_interpolates = self.Dis(interpolates, batch.y)
            ...
            d_loss = self.critic_loss(self.Dis(z, batch.y), self.Dis(gen_output.detach(), batch.y)) + gradient_penalty
            self.log("d_loss", d_loss, prog_bar=True)

            self.manual_backward(d_loss)
            dis_optimizer.step()
            dis_optimizer.zero_grad()
            self.untoggle_optimizer(dis_optimizer)

            # 每5个step更新一次生成器
            if batch_idx % 5 == 0:
                self.toggle_optimizer(gen_optimizer)

                g_loss = self.generator_loss(self.Dis(gen_output, batch.y))
                glabel_loss = self.gan_property_loss(gen_output, batch.y)
                if glabel_loss < 0.01:
                    total_generator_loss = g_loss
                else:
                    total_generator_loss = g_loss + 10 * glabel_loss
                self.log("g_loss", total_generator_loss, prog_bar=True)
    ...
    def validation_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        base_log_dict, base_loss = self.compute_stats(batch, outputs, prefix='val')
    ...
    def test_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        outputs = self(batch, teacher_forcing=False, training=False)
        base_log_dict, base_loss = self.compute_stats(batch, outputs, prefix='test')
