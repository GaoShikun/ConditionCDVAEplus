...
class GemNetT(torch.nn.Module):
    ...
    def __init__(
        ...
        AutomaticFit.reset(self)  # make sure that queue is empty (avoid potential error)
    ...
    def forward(self, z, frac_coords, atom_types, num_atoms, lengths, angles,
        ...
        rad_cbf3, cbf3 = self.cbf_basis3(D_st, cosφ_cab, id3_ca)    #\text{rbf}(D_{ca}) = \exp\left(-\frac{D_{ca}^2}{2\sigma^2}\right)  \text{cbf}(\cos \phi_{cab}) = \text{cos}(\phi_{cab})
