...
def build_crystal(crystal_str, niggli=True, primitive=False):
    ...
    if primitive:
        crystal = crystal.get_primitizve_structure()
...
def build_crystal_graph(crystal, graph_method='crystalnn'):
    ...
    edge_indices, to_jimages = [], []      # edge_indices:边缘索引   jimage:相邻原子在晶胞中的相对位置变化
    if graph_method != 'none':
        for i, j, to_jimage in crystal_graph.graph.edges(data='to_jimage'):
            edge_indices.append([j, i])
            to_jimages.append(to_jimage)
            edge_indices.append([i, j])
            to_jimages.append(tuple(-tj for tj in to_jimage))   # 在无向图中，边的方向并不重要，但相应的位移信息却是对称的，因此需要同时存储两个方向的位移。
...
def frac_to_cart_coords(
    ...
    pos = torch.einsum('bi,bij->bj', frac_coords, lattice_nodes)  # cart coords 将原子由分数坐标转换为的笛卡尔坐标  b:原子索引  i:分数坐标的维度  j:晶格节点的维度

    return pos


def cart_to_frac_coords(    #笛卡尔坐标→分数坐标
    cart_coords,
    lengths,
    angles,
    num_atoms,
    ...
    inv_lattice = torch.linalg.pinv(lattice)            #计算lattice的广义逆矩阵
...
def get_pbc_distances(
    ...
    offsets = torch.einsum('bi,bij->bj', to_jimages.float(), lattice_edges)  #图像索引和晶格边缘计算偏移量
...
def radius_graph_pbc(cart_coords, lengths, angles, num_atoms,
    ...
    num_atoms_per_image_sqr = (num_atoms_per_image ** 2).long()  #每个图像中可能的原子对数量
    ...
    pbc_offsets = torch.bmm(data_cell, unit_cell_batch)         #计算周期性条件下的offsets
    pbc_offsets_per_atom = torch.repeat_interleave(
        pbc_offsets, num_atoms_per_image_sqr, dim=0
    )

    # Expand the positions and indices for the 9 cells
    pos1 = pos1.view(-1, 3, 1).expand(-1, -1, num_cells)     #[batch_size*num_atoms,3,num_cells]
    pos2 = pos2.view(-1, 3, 1).expand(-1, -1, num_cells)
    index1 = index1.view(-1, 1).repeat(1, num_cells).view(-1)       #[batch_size*num_pairs*num_cells]
    ...
    mask_within_radius = torch.le(atom_distance_sqr, radius * radius)       #   atom_distance**2 <＝ radius**2
    # Remove pairs with the same atoms (distance = 0.0)
    mask_not_same = torch.gt(atom_distance_sqr, 0.0001)                #    atom_distance**2 > 0.0001
    mask = torch.logical_and(mask_within_radius, mask_not_same)                 #  0.0001 < atom_distance**2 <= radius**2
...
def preprocess(input_file, num_workers, niggli, primitive, graph_method,
    ...
    unordered_results = p_umap(
        process_one,
        [df.iloc[idx] for idx in range(len(df))],   #len(df)
        [niggli] * len(df),
        [primitive] * len(df),
        [graph_method] * len(df),
        [prop_list] * len(df),
...
def add_scaled_lattice_prop(data_list, lattice_scale_method):
    for dict in data_list:
        ...
        dict['avg_aggregate_num '] = np.mean(num_atoms)
